...
class User(AbstractBaseUser, IsDeletedModel):
    ...
    #поля для управления доступом к административной панели и активностью пользователя.
    is_staff = models.BooleanField(default=False)
    is_active = models.BooleanField(default=True)
    #определяет допустимые значения (SELLER или BUYER).
    account_type = models.CharField(max_length=6, choices=ACCOUNT_TYPE_CHOICES, default="BUYER")

    #указывает, что адрес электронной почты является именем пользователя
    USERNAME_FIELD = "email"
    #указывают, какие поля должны быть заполнены при создании нового пользователя
    REQUIRED_FIELDS = ["first_name", "last_name"]
    #менеджер может добавлять дополнительную логику для работы с пользователями, такую как создание, обновление или аутентификацию.
